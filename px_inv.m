...
q2 = [-pi/6 -pi/4 pi/2 pi/4  ];
L(1) = Link('revolute','alpha', pi/2 , 'a', 0,   'd', l(1) , 'offset', 0 , 'qlim', [-pi pi]);
L(2) = Link('revolute', 'alpha' , 0 ,'a', l(2) , 'd', 0 , 'offset', pi/2, 'qlim',[-pi pi]);
L(3) = Link( 'revolute','alpha', 0 ,    'a',l(3) ,   'd',0 ,   'offset', 0, 'qlim', [-pi pi]);
L(4) = Link( 'revolute','alpha', pi/2,    'a', l(4)  ,   'd',0 ,   'offset', 0, 'qlim', [-pi pi]);
% L(5) = Link( 'revolute','alpha', 0  ,    'a',0 ,   'd',0 ,   'offset', 0 , 'qlim', [-pi pi]);
...
qt= deg2rad([0, 0, 0, 0]);
q = deg2rad([0, 0, 0, 0]);
PhantomX.plot(q2,'notiles','noname');
hold on
